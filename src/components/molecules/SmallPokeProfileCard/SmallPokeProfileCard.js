...
const SmallPokeProfileCard = (props) => {
  ...
  return (
    <Card sx={{border: '2px black', padding: '20px', display: 'flex', flexDirection: 'column', width:'200px', }}>
        <Typography variant="h4" sx={{fontSize: '20px', textAlign: 'center'}}>{pokemon.name.toUpperCase()}</Typography>
        <SpriteContainer>
            <img src={pokemon.sprites.front_default}/>
            <img src={pokemon.sprites.back_default}/>
        </SpriteContainer>
        <Box sx={{display: 'flex'}}>
            <Button onClick={() => addFavoritePokemon(pokemon)} startIcon={<FavoriteBorderIcon/>} sx={{ height: '50px', fontSize: '10px'}}>Favorite</Button>
            <Button startIcon={<AddIcon />} sx={{ height: '50px', fontSize: '10px'}}>Add to Team</Button>
        </Box>
    </Card>
  )
}
